...
def compute_epsilon_dpsgd_noamp(steps: int, noise_multiplier: float, delta: float, verbose: bool = False):
    """Compute epsilon for DP-SGD without privacy amplification.

    This matches the paper's "DP-SGD (no-amp)" baseline: each training step is
    treated as a standard Gaussian mechanism (no subsampling/shuffling
    amplification), and we compose `steps` mechanisms using RDP.

    Under per-example clipping to norm `L` and adding Gaussian noise with
    std = noise_multiplier * L (to the *summed* gradients) before dividing by
    the (expected) batch size, the privacy loss depends only on the
    noise_multiplier. Each step provides (alpha, alpha/(2*nm^2))-RDP.

    Args:
        steps: number of training steps composed.
        noise_multiplier: noise multiplier (sigma / L) used by DP-SGD.
        delta: target delta for (epsilon, delta)-DP.
        verbose: whether to print diagnostic info.

    Returns:
        epsilon achieving (epsilon, delta)-DP.
    """
    if steps <= 0:
        raise ValueError(f"steps must be positive, got {steps}")
    if noise_multiplier <= 0:
        raise ValueError(f"noise_multiplier must be positive, got {noise_multiplier}")
    # Per-step RDP: eps_rdp(alpha) = alpha / (2 * nm^2)
    # Composition over `steps`: steps * alpha / (2 * nm^2)
    # Convert to DP using the same conversion as convert_gaussian_renyi_to_dp.
    alphas = np.arange(1, 200, 0.1)[1:]
    sigma = float(noise_multiplier)
    epss = steps * (alphas / 2 / sigma**2) - (np.log(delta * (alphas - 1)) - alphas * np.log(1 - 1 / alphas)) / (alphas - 1)
    idx = np.nanargmin(epss)
    if verbose and idx == len(alphas) - 1:
        print('The best alpha is the last one. Consider increasing the range of alpha.')
    return float(epss[idx])
